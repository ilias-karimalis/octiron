...
struct printer<T>
{
    ...
    {
        static_assert(m_base >= 2 || m_base <= 16, "Unsupported Base used, must be between 2 and 16.");
...
struct printer<T>
{
    static void print(putCharFunc fn, T value)
    {
        if (value < 0) {
            fn('-');
            printer<typename std::make_unsigned<T>::type>::print(fn, static_cast<std::make_unsigned<T>::type>(-value));
        } else {
            printer<typename std::make_unsigned<T>::type>::print(fn, static_cast<std::make_unsigned<T>::type>(value));
        }
    }
...
/// We allow for hexadecimal printing of unsigned arithmetic types through a specialization of the printer class.
template<Unsigned T>
struct hex
{
...
struct printer<hex<T>>
{
    ...
    {
        static_assert(sizeof(T) <= 8, "Hexadecimal printing is only supported for types up to 64 bits.");
