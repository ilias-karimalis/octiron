...
public:
    ...
    consteval static_array() noexcept = default;

    /// Constructor that takes a single value and copies it into each index of
    /// the array.
    consteval static_array(const T& val) noexcept;
...
template<typename T, size_t N>
consteval static_array<T, N>::static_array(const T& val) noexcept
{
...
// template<typename T, size_t N>
// constexpr T & static_array<T, N>::at(size_t index) {
//     return m_data[index];
// }
//
// template<typename T, size_t N>
// constexpr const T & static_array<T, N>::at(size_t index) const {
// }
